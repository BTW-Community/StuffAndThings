	...
	protected void onImpact(MovingObjectPosition impactPos)
	{
		...
		if ( entityHit != null )
		{
			...
			if( !worldObj.isRemote )
			{
				...
				{
					if(!AttemptToPlaceWebInBlock( iWebI, iWebJ, iWebK ))
					{
						AttemptToPlaceWebInBlock( iWebI, iWebJ+1, iWebK);
					}
				}
			}
		}
		...
		{        	
			if( !worldObj.isRemote )
			{
				...
				if(!AttemptToPlaceWebInBlock( targetPos.i, targetPos.j, targetPos.k ))
				{
					AttemptToPlaceWebInBlock(targetPos.i, targetPos.j+1, targetPos.k);
				}
			}
		}
