...
public class FCBlockKiln extends Block
{
	...
	public void updateTick( World world, int i, int j, int k, Random random )
	{
		...
		if ( CanBlockBeCooked( world, i, j + 1, k ) )
		{
			if ( CheckKilnIntegrity( world, i, j, k ) )
			{
				...
				if
				(
					world.getBlockId(i, j-1, k) == Block.fire.blockID &&
					(world.getBlockId(i, j+1, k) != FCBetterThanWolves.fcUnfiredPottery.blockID || world.getBlockMetadata(i, j+1, k) != FCBlockUnfiredPottery.m_iSubtypeCrucible)
				)
				{
					iNewCookCounter = 0;
				}
	...
	{
		if(blockAccess.getBlockId(i, j-1, k) == Block.fire.blockID)
		{
			return m_iMaxFireFactorBaseTickRate * 2;
		}
