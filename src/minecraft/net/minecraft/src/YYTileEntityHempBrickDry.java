...
public class YYTileEntityHempBrickDry extends TileEntity implements FCITileEntityDataPacketHandler
{
    private final int m_iTimeToCook = 12000;
    private final int m_iRainCookDecay = 10;
    private int m_iCookCounter = 12000;
    ...
    public void UpdateCooking()
    {
        ...
        YYBlockHempBrickDry var6 = YYBaitMod.yyBlockHempBrickDry;

        if (this.m_bIsCooking)
        {
            ++this.m_iCookCounter;

            if (this.m_iCookCounter >= m_iTimeToCook)
            {
                
                this.m_iCookCounter = m_iTimeToCook;
            }
        }
        else if (this.IsRainingOnBrick(this.worldObj, this.xCoord, this.yCoord, this.zCoord))
        {
            this.m_iCookCounter -= 2;
    ...
    private int ComputeCookLevel()
    {
        if (this.m_iCookCounter > 0)
        {
            int var1 = (int)((float)this.m_iCookCounter / m_iTimeToCook * 7.0F) + 1;
